...
double zerovel[NUMOFMANI_DRIVE] = {0}; // initial vel
int32_t actualvel[NUMOFMANI_DRIVE] = {0}; // initial vel
int32_t desinc[NUMOFMANI_DRIVE] = {0};
int32_t targetpos[] = {0,0,0,0,0,0,0,
                        -63715, 38594, 37694, -20069, 85386, -12212, -72300};
int32_t homepos[] = {0,0,0,0,0,0,0,
                     -63715, 38594, 37694, -20069, 85386, -12212, -72300};// for ver1 : 0, for ver2 : each home position {-63715, 38594, 37694, -20069};
//for ver1 i = 0~6, for ver2 i = 7~13
double velprofile[] = {4.0, 4.0, 4.0, 4.0, 4.0, 4.0, 4.0,
...
void motion_callback(const control_msgs::FollowJointTrajectoryActionGoal::ConstPtr& msg)
{
    ...
    for (int j=1;j<traj;++j){
        ...
        for (int i=0; i<NUMOFMANI1_DRIVE; i++)
        {
            ...
            //rt_printf("%i, targetpos = %i, %i,%i\n" ,i, pos_desired_rad[j-1][i], pos_desired[j-1][i],
            //          [i]);
        }
        for (int i=NUMOFMANI1_DRIVE; i<NUMOFMANI_DRIVE; i++)
        {
            ...
            desinc[i] = pos_desired[j-1][i] + homepos[i-NUMOFMANI1_DRIVE];
            //rt_printf("%i, targetpos = %i, %i,%i\n" ,i, pos_desired_rad[j-1][i], pos_desired[j-1][i],
            //          [i]);
        }
        traj_time(desinc);
