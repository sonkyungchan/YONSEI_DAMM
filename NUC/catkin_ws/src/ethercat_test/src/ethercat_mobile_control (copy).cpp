            ...
            rt_printf("%d slaves found and configured.\n", ec_slavecount);
                ...
                for (int k=1; k<(NUMOFWHEEL_DRIVE+1); ++k)
                {
                    if (( ec_slavecount >= 1 ) && (strcmp(ec_slave[k].name,"EPOS4") == 0)) //change name for other drives
                    {
                        ...
                        //// ModeOfOperationDisplay UINT8
                        //os=sizeof(ob); ob = 0x60610008;
                        //wkc_count=ec_SDOwrite(k, 0x1A00, 0x04, 
						//FALSE, os, &ob, EC_TIMEOUTRXM);
                        ...
                        //wkc_count=ec_SDOwrite(k, 0x1A00, 0x06, 
						//FALSE, os, &ob, EC_TIMEOUTRXM);
                    	...
                    	//// 4. Digital outputs UINT32
                    	//os=sizeof(ob); ob = 0x60FE0120;
                    	//wkc_count=ec_SDOwrite(k, 0x1600, 0x04, 
                    	//FALSE, os, &ob, EC_TIMEOUTRXM);
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber input_sub = n.subscribe("input_msg",10, wheel_callback);
