...
int32_t homepos[7] = {-63715, 38594, 37694, -20069, 85386, -72850, -5000}; //need modi 6,7 joint (14bit)
...
double accprofile[] = {25, 25, 25, 80, 80, 80, 80};
...
//double r2i[NUMOFEPOS4_DRIVE] = {rad2inc, rad2inc, rad2inc, rad2inc, rad2inc, rad2inc, rad2inc/16};
//double r2ips[NUMOFEPOS4_DRIVE] = {rpm2ips, rpm2ips, rpm2ips, rpm2ips, rpm2ips, rpm2ips, rpm2ips/16};
//double r2ipss[NUMOFEPOS4_DRIVE] = {rpms2ipss, rpms2ipss, rpms2ipss, rpms2ipss, rpms2ipss, rpms2ipss, rpms2ipss/16};
...
void resol_conv(){
    for (int i=0;i<NUMOFEPOS4_DRIVE;i++){
        resol[i] = pow(2,18);  // axis rot to motor inc
...
//                    wkc_count=ec_SDOwrite(k+1, 0x1600, 0x06, FALSE, os, &ob, EC_TIMEOUTRXM);
                    ...
                    os=sizeof(ob); ob = 0x0186A0;//5F5E100; // Following error window
...
//                        }
//                    }
//                    rt_printf("%i,%i",t1[i],t2[i]);
                    if (abs(c_1[i]) < abs(targetpos[i] - zeropos[i])) {
                        if (gt >=0 && gt <= t1[i]) {
                            p_des = (int) (zeropos[i] + 0.5 * accprofile[i] * gt * gt * rpms2ipss[i]);
                        } else if (gt <= t2[i]) {
                            p_des = (int) (zeropos[i] + c_1[i]*0.5 + (gt - t1[i]) * velprofile[i] * rpm2ips[i]);

                        } else if (gt <= (t1[i] + t2[i])) {
                            p_des = (int) (zeropos[i] + c_1[i]*0.5
                        ...
                        if (gt >= 0 && gt <= t1[i]) {
                            ...
                            p_des = (int) (zeropos[i] + accprofile[i] * rpms2ipss[i] * (- t1[i]* t1[i]
                                                                                        + 2.0 * gt * t1[i]  - 0.5 * gt * gt ));
                        } else {
                            p_des = targetpos[i];
                        }
...
//                    rt_printf("P,I,D = %d / %d / %d  \n" , epos4_drive_pt[i].ptOutParam->PositionP, epos4_drive_pt[i].ptOutParam->PositionI, epos4_drive_pt[i].ptOutParam->PositionD);
//
...
void traj_time(int32_t msgpos[])
{
    for (int i=0; i<NUMOFEPOS4_DRIVE; i++)
    {
        ...
        if (abs(c_1[i])<abs(msgpos[i]-zeropos[i])){
            t1[i] = (int) (abs(velprofile[i]*rpm2ips[i]/(accprofile[i]*rpms2ipss[i])));
...
void motion_callback(const ethercat_test::pos& msg)
{
    for (int i=0; i<NUMOFEPOS4_DRIVE; i++)
    {
        desinc[i] = int(msg.position[i]*resol[i]/360) + homepos[i]; //degree
