...
void EPOS_OP(void *arg)
{
    ...
    while (run)
    {
        ...
        {
	        if ( gt>=2 && gt <4.0 ){
	            ival = (int) 3000*(gt-2.0);
	        }
	        else if ( gt>=4.000 && gt<14.000 ){
		    ival = 6000;
	        }
	        else if ( gt>=14.000 && gt<16.000 ){
		    ival = (int) 6000 - 3000*(gt-14.0);
	        }
	        else if ( gt>=16.000 && gt<18.000 ){
		    ival = 0;
	        }
	        else if ( gt >= 18.000 && gt<20.000){
		    ival = (int) -3000*(gt-18.0);
	        }
	        else if ( gt >=20.000 && gt<27.000){
		    ival = -6000;
	        }
	        else if ( gt>=27.000 && gt<29.000){
		    ival = -6000 + 3000*(gt-27.00);
	        }
            ...
            {    
				...
				{
					rt_printf("Error code (%d): 0x%x \t"
					,i,epos4_drive_pt[i].ptInParam->ErrorCode);
					rt_printf("EtherCAT time : %ld (us) \n",ethercat_time/1000);
					count++;
				}
			}
            gt+=period;
