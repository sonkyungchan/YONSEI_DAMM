//
// Created by mservort on 20. 9. 17..
//
...
boolean ecat_init(void)
{
    ...
    if (ec_init(ecat_ifname))
    {
        ...
        if (ec_config_init(FALSE) > 0)
        {
            ...
            for (int k=1; k<(NUMOFEPOS4_DRIVE+1); ++k)
            {
                ...
                if (( ec_slavecount >= 1 ) && (strcmp(ec_slave[k].name,"EPOS4") == 0)) //change name for other drives
                {
                    ...
                    wkc_count=ec_SDOwrite(k, 0x1A00, 0x05, FALSE, os, &ob, EC_TIMEOUTRXM);
                    // 5. ErrorCode UINT16
...
void EPOS_OP(void *arg)
{
    ...
    while (run)
    {
        ...
        {
            if ( gt>=2 && gt <3.000 ){
                ival = (int) 6000*(gt-2.0);
            }
            else if ( gt>=3.000 && gt<13.000 ){
                ival = 6000;
            }
            else if ( gt>=13.000 && gt<14.000 ){
                ival = (int) 6000 - 6000*(gt-13.0);
            }
            else if ( gt>=14.000 && gt<16.000 ){
                ival = 0;
            }
            else if ( gt >= 16.000 && gt<17.000){
                ival = (int) -6000*(gt-16.0);
            }
            else if ( gt >=17.000 && gt<27.000){
                ival = -6000;
            }
            else if ( gt>=27.000 && gt<28.000){
                ival = -6000 + 6000*(gt-27.00);
            }
            else if ( gt>=28.000 && gt<30.000){
                ival = 0;
            }
            else if ( gt>=30.000){
                ival = 0;
            ...
            epos4_drive_pt[0].ptOutParam->TargetVelocity=ival;
            epos4_drive_pt[1].ptOutParam->TargetVelocity=ival;
            epos4_drive_pt[2].ptOutParam->TargetVelocity=ival;
            //epos4_drive_pt[3].ptOutParam->TargetVelocity=ival;
...
//    char** argv;
    ...
    motorsMsg measure_msg;

//    ros::init(argc,argv,"dualarm_pub");
//    ros::NodeHandle n;

//    ros::Publisher pub = n.advertise<motorsMsg>("measure",1);
    ...
    while (run)
    {
        ...
        if (inOP==TRUE)
        {
            ...
            else
            {
                ...
                //measure_msg.omega1 = epos4_drive_pt[0].ptInParam -> VelocityActualValue;
                //measure_msg.omega2 = epos4_drive_pt[1].ptInParam -> VelocityActualValue;
                //measure_msg.omega3 = epos4_drive_pt[2].ptInParam -> VelocityActualValue;
...
int main(int argc, char *argv[])
{
    ...
    rt_task_create(&motion_task, "SOEM_motion_task", 0, 96, 0 );
    rt_task_set_affinity(&motion_task, &cpu_set_ecat); //CPU affinity for ethercat task

    rt_task_create(&pub_task, "ec_printing", 0, 50, 0 );
    rt_task_set_affinity(&pub_task, &cpu_set_print); //CPU affinity for printing task
