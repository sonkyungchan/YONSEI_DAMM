...
unsigned int cycle_ns = 2e6; // nanosecond
            ...
            rt_printf("%d slaves found and configured.\n", ec_slavecount);
                ...
                for (int k=1; k<(NUMOFWHEEL_DRIVE+1); ++k)
                {
                    if (( ec_slavecount >= 1 ) && (strcmp(ec_slave[k].name,"EPOS4") == 0)) //change name for other drives
                    {
                        ...
                        // 4. DigitalInput(LogicState) UINT32
                        ...
                        wkc_count=ec_SDOwrite(k, 0x1A00, 0x05, 
                        FALSE, os, &ob, EC_TIMEOUTRXM);
                    ...
                    wkc_count=ec_SDOwrite(k, 0x1600, 0x03, 
                    FALSE, os, &ob, EC_TIMEOUTRXM);
        ...
        if (sys_ready)
        {
            ...
            for (i=0; i<NUMOFWHEEL_DRIVE; ++i)
            {
				...
				if(epos4_drive_pt[i].ptInParam->ErrorCode != 0)
                    rt_printf("Error : 0x%x %d\n",epos4_drive_pt[i].ptInParam->ErrorCode, i+1);
            }
