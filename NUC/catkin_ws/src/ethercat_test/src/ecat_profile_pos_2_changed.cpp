...
// This is for manipulator ver 2
// initial value changed on 21. 01. 29
// Change velocity to relative moving, 5th homeposition
// on 21. 08. 23
...
double velprofile[] = {VELMAX, VELMAX, VELMAX, VELMAX, VELMAX, VELMAX, VELMAX}; // **
//double velprofile[] = {2., 1.5, 2.5, 2.5, 3., 3., 3.}; // axis rpm all 1.5
double accprofile[] = {50.0, 50.0, 80.0, 80.0, 80.0, 80.0, 80.0}; //
int foredes[NUMOFEPOS4_DRIVE] = {0};  //**
double desvel[NUMOFEPOS4_DRIVE] = {0};  //**
...
//                    }// using queue for trajectory end
                    ...
                    if (targetpos[i] >= zeropos[i]) {
                        velprofile[i] = abs(VELMAX)*abs(desvel[i]);
...
//        desinc[i] = msg.position[i] + homepos[i];
        ...
        foredes[i] = epos4_drive_pt[i].ptInParam->PositionActualValue; //**
        desinc[i] = int (msg.traj[i]*resol[i]/360.0)+ homepos[i];

//        desinc[i] = int (msg.traj[i] * resol[i] / 2 / M_PI) + homepos[i];
        maxinc = std::max<int>(abs(maxinc),abs(desinc[i]-foredes[i])); //**
...
//        rt_printf("%i, targetpos = %i,%i\n" ,i, msg.position[i],homepos[i]);
    ...
    for (int i=0; i<NUMOFEPOS4_DRIVE; i++)
    {
        desvel[i] = double(abs(desinc[i]-foredes[i])/maxinc); //**
...
//    ros::Subscriber pos_sub = n.subscribe("ourarm/robotic_arm_controller/follow_joint_trajectory/goal", 1, motion_callback);
    ros::Subscriber pos_sub = n.subscribe("arm2", 1, motion_callback);
