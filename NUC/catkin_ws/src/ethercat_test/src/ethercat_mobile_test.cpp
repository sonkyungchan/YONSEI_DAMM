...
#define NUMOFEPOS4_DRIVE 4
#define NSEC_PER_SEC 1000000000

unsigned int cycle_ns = 1e6; // 1 ms
        ...
        if (ec_config_init(FALSE) > 0)
        {
            ...
            for (int k=1; k<(NUMOFEPOS4_DRIVE+1); ++k)
            {
                ...
                if (( ec_slavecount >= 1 ) && (strcmp(ec_slave[k].name,"EPOS4") == 0)) //change name for other drives
                {
                    ...
                    wkc_count=ec_SDOwrite(k, 0x1A00, 
                    0x01, FALSE, os, &ob, EC_TIMEOUTRXM);

                    // 2. PositionActualValue INT32
                    os=sizeof(ob); ob = 0x60640020;
                    wkc_count=ec_SDOwrite(k, 0x1A00, 
                    0x02, FALSE, os, &ob, EC_TIMEOUTRXM);

                    // 3. VelocityActualValue INT32
                    os=sizeof(ob); ob = 0x606C0020;
                    wkc_count=ec_SDOwrite(k, 0x1A00, 
                    0x03, FALSE, os, &ob, EC_TIMEOUTRXM);
    
                    // 4. DigitalInput UINT32
                    os=sizeof(ob); ob = 0x60FD0020;
                    wkc_count=ec_SDOwrite(k, 0x1A00, 
                    0x04, FALSE, os, &ob, EC_TIMEOUTRXM);

                    // 5. ErrorCode UINT16
                    os=sizeof(ob); ob = 0x603F0010;
                    wkc_count=ec_SDOwrite(k, 0x1A00, 
                    0x05, FALSE, os, &ob, EC_TIMEOUTRXM);

                    os=sizeof(ob3); ob3 = 0x05;
                    wkc_count=ec_SDOwrite(k, 0x1A00, 
                    0x00, FALSE, os, &ob3, EC_TIMEOUTRXM);
                    ...
                    wkc_count=ec_SDOwrite(k, 0x1600, 
                    0x01, FALSE, os, &ob, EC_TIMEOUTRXM);
...
void EPOS_OP(void *arg)
{
	...
	//		wkc = ec_receive_processdata(EC_TIMEOUTRET);            
        ...
        if (sys_ready)
        {
            ...
            {   
                epos4_drive_pt[i].ptOutParam->TargetVelocity = ival;
    ...
    while (run)
    {
        ...
        if (inOP==TRUE)
        {
            ...
            else
            {
                ...
                rt_printf("Time = %06d.%01d, \e[32;1m fail=%ld\e[0m, ecat_T=%ld, maxT=%ld, toff=%ld\n",
                        itime/10, itime%10, recv_fail_cnt, ethercat_time/1000, worst_time/1000,toff_);
				for(i=0; i<NUMOFEPOS4_DRIVE; ++i)
                {
...
int main(int argc, char *argv[])
{
 ...
 //   CPU_SET(1, &cpu_set_print); //assign CPU#1 (or any) for main task
    ...
    rt_task_create(&motion_task, "SOEM_motion_task", 0, 97, 0 );
