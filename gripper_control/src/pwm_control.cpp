...
GroupSyncWrite groupSyncWrite(portHandler, packetHandler, ADDR_GOAL_PWM, 2);
...
void syncSetPwmCallback(const dynamixel_sdk_examples::ActuatorPwm::ConstPtr & msg)
{
  uint8_t dxl_error = 0;
  int dxl_comm_result = COMM_TX_FAIL;
  int dxl_addparam_result = false;
  uint8_t param_goal_pwm1[2];
  uint8_t param_goal_pwm2[2];

  // Position Value of X series is 4 byte data. For AX & MX(1.0) use 2 byte data(uint16_t) for the Position Value.
  int16_t pwm1 = msg->pwm1; // Convert int32 -> uint32
  param_goal_pwm1[0] = DXL_LOBYTE(DXL_LOWORD(pwm1));
  param_goal_pwm1[1] = DXL_HIBYTE(DXL_LOWORD(pwm1));


  int16_t pwm2 = msg->pwm2; // Convert int32 -> uint32
  param_goal_pwm2[0] = DXL_LOBYTE(DXL_LOWORD(pwm2));
  param_goal_pwm2[1] = DXL_HIBYTE(DXL_LOWORD(pwm2));

  dxl_addparam_result = groupSyncWrite.addParam((uint8_t)msg->id1, param_goal_pwm1);
  if (dxl_addparam_result != true) {
    ROS_ERROR( "Failed to addparam to groupSyncWrite for Dynamixel ID %d", msg->id1);
  }

  dxl_addparam_result = groupSyncWrite.addParam((uint8_t)msg->id2, param_goal_pwm2);
  if (dxl_addparam_result != true) {
...
{  
  ...
  ros::Subscriber sync_set_position_sub = nh.subscribe("/sync_set_pwm", 1, syncSetPwmCallback);
